        ...
        with open(outPrefix,'w') as g:                                  #write outFile as g
            ...
            for line in f:                                              #lines in inFile
                ...
                if len(currRead)==4:                                    #stop when currRead has 4 items in list
                    UMI=currRead[1][:4]+currRead[1][-6:]                #UMI = first 4 and last 6 chars of line 1. line 1 is 1-index in currReadw
                    #line 1 contains the read. line 3 contains the quality score.
                    if reads[currRead[1][4:-6]][UMI]==0:                #check if we've seen this read+UMI before
                        ...
                        if len(currRead[1])!=len(currRead[3]):          #only want reads the same length as quality score
                            print(currRead, sys.exit())
                    reads[currRead[1][4:-6]][UMI]+=1                    #mark this one as done
                    currRead=[]                                         #clear list
...
if __name__=='__main__':
    Tee()
    main(sys.argv[1:]) #feed everything 1-index and beyond into main
