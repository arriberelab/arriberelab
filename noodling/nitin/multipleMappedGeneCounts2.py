...
Input: inFile.jam 
Output: pickled dataframe of genecount 

Run as python3 multipleMappedGeneCounts2.py inFile.jam outFile.pkl
            ...
            for line in f:
                ...
                if "AS" not in str(senseAntisenseList):
                    ...
                    if lengthRead in range(15,19):
                    	if wbGeneID not in self.geneCountsDict.keys():
                    		self.geneCountsDict.update({wbGeneID:float(1/numberOfDifferentGenesMapped)}) #we add a fractional read count if the read maps to multiple different genes
                    	elif wbGeneID in self.geneCountsDict.keys():
                    		self.geneCountsDict[wbGeneID]+=float(1/numberOfDifferentGenesMapped)
