...
run as python3 plotGeneCts_plotly.py inFile.geneCt outputFile(.pdf/.svg) colName1_y colName2_x ski_pelo_list.txt
"""
import sys
...
def pdParseGeneCtFile(inFile, ski_pelo_list_path):
    import pandas as pd
    dataframe = pd.read_csv(inFile, sep="\t")
    dataframe.rename(columns={dataframe.columns[0]: "gene_id"}, inplace=True)

    ski_pelo_list = pd.read_csv(ski_pelo_list_path, names=['gene_id']).gene_id.to_list()
    dataframe['ski_pelo_targ'] = dataframe['gene_id'].isin(ski_pelo_list)
    ...
    if len(columns_to_plot) == 2:
        ...
        for plot_num, x_key in enumerate(columns_to_plot[1:]):
            ...
            fig.add_trace(go.Scatter(x=geneCtDF[x_key], y=geneCtDF[y_key],
                                     marker_color=geneCtDF['ski_pelo_targ'],
...
def main_plotly_and_pandas(args):
    ...
    columns_to_plot = args[2:-2]
    ski_pelo_path_arg = args[-1]
    df = pdParseGeneCtFile(inFile, ski_pelo_path_arg)
    print(df.info())
    plotlyMkScatterPlots(df, outFile, columns_to_plot)


if __name__ == '__main__':
    # Tee()  # Screw Tee for a mini script like this!
