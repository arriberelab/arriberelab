...
Script to collapse reads containing the exact same sequence. Will pick the read information
    from the first occurence of that read for the outFile. Use this if you have a UMI. 
    
    Use readCollapser3.py if you DO NOT have a UMI!!
    Use readCollapser5.py if you are trimming 6N on 3end and 4N on 5end.

Input: reads.fastq - fastq-formatted reads

Output: reads.collapsed.fastq - same as input, with redundant reads removed

run as python readCollapser4.py inFile.fastq outPrefix
"""
            ...
            for line in f:
                ...
                if len(currRead)==4:
                    ...
                    if reads[currRead[1][:-6]][UMI]==0:
                        ...
                        #g.write('%s\n%s\n%s\n%s\n'%(currRead[0],currRead[1][3:][:-6],currRead[2],currRead[3][3:][:-6]))
